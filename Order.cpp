...
void Order::PayReserve(int number)
{
	...
	cout << cinema->films[film - 1].date[day];
	cout << cinema->films[film - 1].time[time];
	...
	*/
	...
	if (card)
	{
		...
		*/
		...
		cout << "|\n|               Оплата               |\n";
		cout << "|              ОДОБРЕНО              |\n";
		cout << "|СУММА: " << cost * num - sale * num << ".00 RUB";
...
void Order::Tickets()
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		*/
		...
		cout << "\n|ДАТА " << Time::RetDate(day - 1, 1) << "    НАЧАЛО " << cinema->films[film - 1].time[(day - 1) * 3 + time - 1] << "  | Л |";
		cout << "\n|                                 | Ь |";
